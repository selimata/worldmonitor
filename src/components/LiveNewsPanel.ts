...
// Channel tables live in shared/live-channels-data.ts so server-side code (the
// iOS bundle endpoint) can read the same registry the dashboard renders —
// `server/` must not import `src/components/`. Re-exported below so existing
...
export { OPTIONAL_LIVE_CHANNELS, OPTIONAL_CHANNEL_REGIONS };
