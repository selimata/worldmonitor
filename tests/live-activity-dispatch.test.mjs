...
describe('ais-relay wiring (source assertions)', () => {
  ...
  it('observes CRITICAL alerts from both the LLM branch and cached hits, after the notification publish', () => {
    ...
    const llmHook = relaySrc.indexOf("if (level === 'critical') liveActivityObserve(chunk[idx], meta);", publishAt);
    ...
    assert.match(relaySrc, /if \(level === 'critical'\) liveActivityObserve\(titleArr\[i\], allTitles\.get\(titleArr\[i\]\)\);/);
  });
